...
class UnifiedToolHandler:
        ...
        try:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    # 尝试提取第一个有效的JSON对象（处理多个JSON拼接的情况）
                    self.logger.warning(f"JSON解析失败，尝试提取第一个有效JSON对象: {arguments}")
                    arguments = self._extract_first_json(arguments)
    ...
    def _extract_first_json(self, text: str):
        ...
        Args:
            text: 可能包含多个JSON对象的字符串

        Returns:
            第一个有效的JSON对象（字典），如果提取失败则返回None
        """
        if not text:
            return None
