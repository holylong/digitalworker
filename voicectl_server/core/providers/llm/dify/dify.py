...
class LLMProvider(LLMProviderBase):
    ...
    def response_with_functions(self, session_id, dialogue, functions=None):
        if len(dialogue) == 2 and functions is not None and len(functions) > 0:
            ...
            # 添加日志：查找self_schedule_meeting工具
            schedule_meeting_found = False
            for func in functions:
                if isinstance(func, dict) and "function" in func:
                    func_name = func["function"].get("name", "")
                    if "schedule_meeting" in func_name:
                        schedule_meeting_found = True
                        self.logger.info(f"找到预约会议工具: {func_name}")
                        self.logger.info(f"工具描述: {json.dumps(func, ensure_ascii=False)}")
                        break
            
            if not schedule_meeting_found:
                self.logger.warning(f"未找到预约会议工具，可用工具数量: {len(functions)}")
