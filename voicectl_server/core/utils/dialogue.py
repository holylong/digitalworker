...
class Dialogue:
    ...
    def check_and_clear_history(self, timeout_minutes: int, logger=None):
        """
        检查是否超时并清空对话历史

        Args:
            timeout_minutes: 超时时间（分钟），0表示不清空
            logger: 日志记录器（可选）

        Returns:
            bool: 如果清空了历史记录返回True，否则返回False
        """
        # 如果超时时间设置为0，表示不自动清空
        ...
        # 计算距离最后一次用户消息的时间（秒）
        elapsed_time = time.time() - self.last_user_message_time
        ...
        if elapsed_time >= timeout_seconds:
            if logger:
                logger.info(
                    f"对话历史超时（{elapsed_time:.1f}秒 > {timeout_seconds}秒），清空历史记录"
                )
            self.clear_history(keep_system_message=True)
            return True
    ...
    def clear_history(self, keep_system_message: bool = True):
        ...
        if keep_system_message:
            # 保留系统消息
            system_messages = [
                msg for msg in self.dialogue if msg.role == "system"
            ]
            self.dialogue = system_messages
        else:
            # 清空所有消息
            self.dialogue = []
        # 重置最后消息时间
    ...
    def get_llm_dialogue_with_memory(
        ...
        if system_message:
            ...
            # 检查是否有终端模式状态
            # 从外部传入的终端模式状态
            terminal_mode = voiceprint_config.get("terminal_mode", "work") if voiceprint_config else "work"
            if terminal_mode == "sleep":
                enhanced_system_prompt += "\n\n【当前处于休息模式，请判断用户消息是否为唤醒意图】"
