...
def pcm_to_data(raw_data, is_opus=True):
    ...
    # 使用单例编码器，避免重复创建
    encoder = OpusEncoderUtils.get_instance()
    
    # 编码 PCM 数据为 Opus
    opus_packets = encoder.encode_pcm_to_opus(raw_data, end_of_stream=True)
...
def opus_datas_to_wav_bytes(opus_datas, sample_rate=16000, channels=1):
    ...
    """
    # 使用单例解码器，避免重复创建
    from core.utils.opus_decoder_utils import OpusDecoderUtils
    
    decoder = OpusDecoderUtils.get_instance()
    pcm_data = []

    for opus_frame in opus_datas:
        # 解码为PCM（返回bytes，2字节/采样点）
        pcm = decoder.decode_opus_to_pcm(opus_frame)
        if pcm:
            pcm_data.append(pcm)
