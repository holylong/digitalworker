...
def send_email(api_url, to_email, subject, content):
    ...
    try:
        ...
        return {
            "status_code": 500,
            "code": 500,
            "message": f"连接失败：无法连接到邮件服务 ({str(e)})"
        }
...
def get_contact_by_name(api_url, name):
    ...
    try:
        ...
        # 发送GET请求，设置超时时间为5秒，避免长时间等待
        response = requests.get(f"{api_url}/email/{name}", headers=headers, timeout=5)
